...
import { PARTNERS, PAYMENT_METHODS, PICKUP_SLOTS } from '@/lib/data'
...
export function CheckoutScreen() {
  ...
  const canPlace = Boolean(pickupSlot && payment && providerId)

  function handlePlace() {
    if (!canPlace) {
      toast('Pick a shop, slot and payment method', 'error')
      return
    }
    setPlacing(true)
    setTimeout(() => {
  ...
  return (
    <div className="min-h-dvh bg-background pb-40">
      ...
      <div className="space-y-5 px-4 py-4">
        {/* Shop Selection */}
        <Section title="Select Care Shop">
            ...
            {PARTNERS.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`flex min-w-[200px] flex-col gap-2 rounded-2xl border p-4 text-left transition-all ${
                    active ? 'border-primary bg-primary/5 shadow-sm' : 'border-border bg-card'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className={`flex size-10 items-center justify-center rounded-xl font-bold ${active ? 'bg-primary text-white' : 'bg-muted text-primary'}`}>
                      {p.name.slice(0, 2)}
                    </span>
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">{p.turnaround}</p>
                  </div>
          ...
          <div className="space-y-2">
            ...
            {(!user?.addresses || user.addresses.length === 0) && (
              <button
                ...
                className="w-full rounded-2xl border border-dashed border-border p-4 text-center text-sm font-medium text-muted-foreground hover:text-primary transition-colors"
              >
